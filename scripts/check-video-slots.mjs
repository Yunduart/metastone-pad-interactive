...
const total = CONTENT_CATALOGS.reduce(
  (sum, catalog) => sum + catalog.items.reduce((catalogSum, item) => catalogSum + item.playlist.length, 0),
  0,
);
let ready = 0;
...
for (const catalog of CONTENT_CATALOGS) {
  ...
  for (const item of catalog.items) {
    console.log(`  ${item.marker} ${item.title} · ${item.playlist.length} 项`);
    for (const [index, media] of item.playlist.entries()) {
      const absolutePath = resolve(videoRoot, media.fileName);
      ...
      const hint = hasDuplicatedExtension ? `（检测到 ${media.fileName}.mp4，请删除多余的 .mp4）` : "";
      const loop = media.loop ? " · LOOP" : "";
      console.log(`    ${status} ${String(index + 1).padStart(2, "0")} ${media.title}${loop} -> ${media.fileName}${size}${hint}`);
    }
  }
  console.log("");
}

console.log(`播放项状态：${ready}/${total} 已放入，${total - ready}/${total} 等待素材。`);
